...
export default class CollisionHandler {
    handleShellCollision(collisionBlock: Tank | obstacleType | Base | undefined, shell: Shell, field: Field) {
    ...
    deathHandler(collisionBlock: Base | Tank | obstacleType, field: Field) {
        ...
        if (
            ...
            const parentCollection = field.getParentCollection(collisionBlock)
    ...
    respawnEntity(entity: Tank, field: Field) {
        if (entity.respawnCount > 0) {
            ...
            const parentCollection = field.getParentCollection(entity)
    ...
    shellToParticle(shell: Shell, field: Field) {
        ...
        const particle = new Particle(spawnPoint.x, spawnPoint.y, shell.direction)
