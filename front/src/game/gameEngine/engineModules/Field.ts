...
class Field {
  ...
  getMinimalStep(step: Point, gameObject: GameObject) {
    ...
    while (
      (collisionBlock &&
        this.hasObstacleCollision(gameObject, minimalStep, collisionBlock)) ||
    ...
    if (
      collisionBlock &&
      gameObject.constructor.name === "Shell" &&
      minimalStep.x === 0 &&
      minimalStep.y === 0
    ) {
        ...
        if (collisionBlock.hp === 0) {
          if ("team" in collisionBlock) {
            ...
            parentCollection.splice(
              parentCollection.indexOf(collisionBlock),
              1
            );
          }
        }
  ...
  findCollisionBlock(minimalStep: Point, gameObject: GameObject) {
    return (
      this.obstacle.find((obstacle) =>
        this.hasObstacleCollision(gameObject, minimalStep, obstacle)
      ) ||
      this.base.find((base) =>
        this.hasObstacleCollision(gameObject, minimalStep, base)
      ) ||
      this.tanks.find(
        (tank) =>
          tank !== gameObject &&
          this.hasObstacleCollision(gameObject, minimalStep, tank)
      )
    );
  }
  ...
  hasIntersects(
    firstRect: GameObject,
    step: Point,
    secondRect: GameObject
  ): boolean {
