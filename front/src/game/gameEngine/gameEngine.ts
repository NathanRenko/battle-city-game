...
import {
  directionToAngle,
  entityDirections,
} from "./engineModules/constObjects/DirectionHandler";
...
class GameEngine {
  ...
  finishGame() {
    ...
    if (Store.socket) {
      setTimeout("Store.socket.disconnect", 1000);
    } else {
  ...
  draw() {
    ...
    for (const entityType in EntityClasses) {
      ...
      const isDirectionable =
        entityCollection.length !== 0 && "direction" in entityCollection[0];
  ...
  drawHpBar(context: CanvasRenderingContext2D, entity: Tank | Base) {
    ...
    if ("direction" in entity) {
      ...
      context.fillRect(
        entity.x,
        entity.y + entity.size + margin,
        hpPercent * entity.size,
        hpBarH
      );
    }
    // @ts-ignore
